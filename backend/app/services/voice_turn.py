...
HARD_RED_CODES = {"TRAUMA_AMPUTATION", "RESP_SEVERE", "NEURO_SEVERE", "BLEEDING_SEVERE", "CARDIAC_ALERT"}


def classify_triage_with_codes(text: str) -> tuple[str, list[str]]:
    s = text.lower().strip()
    score = {"RED": 0, "AMBER": 0}
    ...
    if any(code in HARD_RED_CODES for code in matched_codes) or score["RED"] >= 4:
        return "RED", matched_codes
    if score["AMBER"] > 0:
        return "AMBER", matched_codes
    return "GREEN", matched_codes
...
async def run_voice_turn(
    ...
    triage_level, triage_reason_codes = classify_triage_with_codes(transcript)
    ...
    assistant_message = (
        _try_generate_with_openai(transcript, triage_level, triage_reason_codes, top_places)
